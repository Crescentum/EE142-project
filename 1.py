...
class InfoGANTrainer:
    ...
    def __init__(self, cfg: TrainerConfig):
        ...
        # N_CATS: number of categorical code groups (default 1 for MNIST)
        ...
        # WGAN-GP: replace D head (remove sigmoid)
        if cfg.use_wgan_gp:
            # Dynamically determine the input dimension of d_head
            # For MNIST: shared_fc outputs 1024; for SVHN: shared_conv+flatten outputs 4096
            # We inspect the existing d_head to get the input dim
    ...
    def _make_fixed_latents(self):
        ...
        # For multiple categorical codes: only vary the first code across rows
        # Each row has a different class for c1, all other codes fixed to class 0
        c_cat = torch.zeros(B, N_CATS * CAT_DIM, device=device)
        for i in range(10):
            # Set first categorical code to class i
            c_cat[i*10:(i+1)*10, i] = 1.0
            # Other categorical codes remain at class 0 (already zero, one-hot at index 0)
    ...
    def _step(self, real_imgs):
        ...
        with torch.no_grad():
            # L_I lower bound for discrete codes: H(c) - MI_loss
            # For multiple codes, target is log(CAT_DIM) per code
    ...
    def _visualise(self, epoch):
        ...
        z = self.concat_latent(self.fixed_noise, self.fixed_c_cat,
                                self.fixed_c_cont)
        imgs = self.G(z)
        # Normalize for display: SVHN uses Tanh -> [-1,1], MNIST uses Sigmoid -> [0,1]
        if imgs.min() < 0:
        ...
        # Fixed latent for continuous traversal: first categorical code = class 0
        c0 = torch.zeros(10, N_CATS * CAT_DIM, device=device)
        c0[:, 0] = 1.0  # first code class 0
        for j in range(1, N_CATS):
            c0[:, j * CAT_DIM] = 1.0  # other codes class 0

        zn = torch.zeros(10, NOISE_DIM, device=device)
        sweep = torch.linspace(-2, 2, 10, device=device)

        # Traverse each continuous code
